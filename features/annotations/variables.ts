...
const coordinates: { x: number; y: number } = JSON.parse(json); //JSON.parse가 any type을 반환함
//any type : JSON에서 어떤 유형의 값이 반환되는지 Typescript는 모름
console.log(coordinates);

//2 한줄에 변수 선언 다른 줄에 변수 초기화
let words = ['red', 'green', 'yellow'];
let foundWord: boolean; // annotation 추가
...
for (let i = 0; i < numbers.length; i++) {
  if (numbers[i] > 0) numberAboveZero = numbers[i]; // error: Type 'boolean' is not assignable to type 'number'.
}
