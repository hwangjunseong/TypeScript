...
const numResults: Array<number> = []; //숫자로 이뤄진 배열

const textResults: string[] = []; //문자열로 이뤄진 배열

//type 별칭 => js에는 없는 typescript 의 type
//반복 피하고싶어서 사용
//=> type 별칭은 객체 유형을 저장할 떄 사용가능
type NumOrString = number | string;
type Result = { val: number; timestamp: Date };
//객체 유형의 타입을 다룰 때 또 다른 대안 :interface
//객체의 구조를 정의할 수 있다
interface ResultObj {
  val: number;
  timestamp: Date;
